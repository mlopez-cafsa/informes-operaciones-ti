...
| 1.19 | 2026-09-23 | Validación de forma para `data/informes.json` y `data/pendientes.json` (a pedido de Marco, tras revisar patrones de software aplicables al sistema). Nuevo `scripts/modelos.py`: dos `dataclasses` (`Informe`, `Pendiente`) sin dependencias externas, con validación en `__post_init__()` — campos obligatorios, valores permitidos de `prioridad`/`cumplimiento`/`estado`/`criticidad`/`estado_item`, formato de fecha `YYYY-MM-DD`, y forma de `fases`/`jira_urls`/`subtareas`. `cargar_informes()`/`guardar_informes()` (`manage_informes.py`) y `cargar_pendientes()`/`guardar_pendientes()` (`reportes_lib.py`) corren esta validación y detienen la ejecución con un `[ERROR]` claro si un registro no calza, en vez de dejar que el problema aparezca después como HTML roto. El resto del código sigue trabajando con `dict` normales — las dataclasses se usan solo como validador en el borde de entrada/salida, no se propagan al resto del sistema. Verificado con los 8 informes y 2 pendientes reales (pasan sin error) y con 5 casos de dato inválido a propósito (prioridad con typo, fecha mal formada, campo obligatorio faltante, clave inesperada, número fuera de rango) — los 5 se detectan con mensaje claro. Regeneración completa del sitio y balance de HTML (12 páginas) verificados sin errores tras el cambio. |
